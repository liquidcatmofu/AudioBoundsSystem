...
| Automated tests | Partial | 93 pure-Java tests pass; four Forge GameTests boot a headless 1.20.1 server and verify BlockEntity creation, NBT round trips and Controller-to-Speaker playback timing | Test sources under `common`, `tts-addon` and `forge/src/gametest` | Player packet delivery, browser RPC and multi-client paths remain uncovered | Add packet integration tests |
