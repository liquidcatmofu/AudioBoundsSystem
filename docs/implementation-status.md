...
|---|---|---|---|---|---|
| Web dashboard and authentication | Implemented, partially tested | Tests cover session expiry/invalidation and verify that the loopback client server rejects unauthenticated API requests with a non-cacheable 401 response | `ClientWebServer`, `WebRpcClient`, `WebRpcService`, `WebApiRouter`, `WebSessionStore` | Full browser and multiplayer RPC verification remains | Run authenticated identity, mutation and disconnect acceptance tests |
