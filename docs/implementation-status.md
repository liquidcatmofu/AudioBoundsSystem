...
|---|---|---|---|---|---|
| Web dashboard and authentication | Implemented, partially tested | HTTP tests verify loopback-only binding, unauthenticated 401, rotating auth URLs, HttpOnly/SameSite cookies and authenticated WebUI delivery; session expiry/invalidation is also covered | `ClientWebServer`, `WebRpcClient`, `WebRpcService`, `WebApiRouter`, `WebSessionStore` | Full browser and multiplayer RPC verification remains | Run authenticated identity, mutation and disconnect acceptance tests |
...
| WAV/audio-to-Ogg transcoding | Verified working | User confirmed operation after Core/TTS consolidation; tests cover bit-exact command flags plus process success, timeout escalation and interruption cleanup | `FfmpegTranscoder`, `ExternalProcessRunner` | 90-second timeout is fixed; a real failed FFmpeg invocation remains manual | Verify a configured non-default FFmpeg path |
...
| TTS cache | Implemented but untested | The addon checks a deterministic, format-versioned full-request cache before Provider synthesis; validated atomic entries use a 128 MiB access-time LRU and same-key requests are serialized | `AddonTTSBridge`, `TTSAudioCache`, `DiskCachePruner` | Format version must be incremented when synthesis output encoding changes | Verify eviction and cache hits with a real Provider |
| Server lifecycle | Implemented, partially tested | Audio transfer, Web RPC and library-maintenance services have idempotent start/stop/restart tests; FFmpeg conversion and probes share tested termination handling; owned executors stop with the server | `ABSServerLifecycle`, `AudioTransferService`, `WebRpcService`, `FfmpegSupport`, `LibraryCacheMaintenance` | Full server runtime restart remains unverified; provider bridge has no shutdown hook | Verify a full server restart |
| Client lifecycle | Implemented, partially tested | Loopback WebUI server start/stop/restart is tested; active sound/subtitle state and in-flight downloads clear on player quit | `AudioBoundsSystemClient`, `ClientWebServer`, `SpeakerAudioManager` | Download executor lives for the client process, although its threads are daemon threads | Add disconnect-during-download integration test |
