...
| Playback sequence | Implemented, partially tested | Sequence metadata and controller expansion are present; pure tests cover PULSE/LEVEL transitions and STOP/RESTART retriggers | `LibrarySequence`, `AudioControllerBlockEntity`, `ControllerSignalTransition` | Queue expansion and timed playback remain uncovered | Extract pure queue planning logic for tests |
