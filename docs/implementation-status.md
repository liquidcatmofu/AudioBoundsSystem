...
| One-time audio delivery | Implemented, partially tested | Per-player tokens reject cross-player consume/discard; pure tests verify token lifecycle, cache-hit revocation, playback metadata and bounded transfer packet round trips; busy/stopped services reject before consuming the token | `TokenStore`, `PlayAudioPacket`, `AudioTransferChunkPacket`, `AudioTransferService` | Live Minecraft packet delivery and retry remain untested | Verify repeated playback and busy retry with two clients |
