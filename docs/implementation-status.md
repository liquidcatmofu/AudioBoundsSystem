...
| WebUI library configuration | Implemented but untested on Minecraft RPC | JSON, preview and upload bodies use bounded chunked RPC while existing authorization handlers are reused; bodies above 64 KiB are assembled in temporary files and streamed through source storage into FFmpeg | `LibraryApiHandler`, `WebRpcRequestBody`, `WebRpcProtocol`, `webui/app.js` | 64 KiB JSON and 64 MiB upload limits are fixed | Run upload, timeout and disconnect acceptance tests on Windows and Unix-like hosts |
...
| Playback sequence | Implemented, partially tested | Pure tests cover PULSE/LEVEL transitions, STOP/RESTART retriggers, ordered sequence expansion, due-tick boundaries and LEVEL loop completion; Controller playback honors each sequence step's post-track delay | `LibrarySequence`, `AudioControllerBlockEntity`, `ControllerSignalTransition`, `ControllerQueuePlan`, `ControllerQueueTiming` | Playback side effects remain uncovered | Add headless integration coverage with controlled audio fixtures |
