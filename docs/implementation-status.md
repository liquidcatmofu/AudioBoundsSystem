...
| Automated tests | Partial | 48 pure-Java tests pass; Forge GameTest boots a headless 1.20.1 server and verifies Speaker BlockEntity creation | Test sources under `common`, `tts-addon` and `forge/src/gametest` | Actual playback, browser RPC and multi-client paths remain uncovered | Add focused BlockEntity persistence and controller tests |
