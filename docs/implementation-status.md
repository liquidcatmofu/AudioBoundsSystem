...
|---|---|---|---|---|---|
| Web dashboard and authentication | Implemented, partially tested | Loopback HTTP tests cover unauthenticated 401 plus one-time auth URL, HttpOnly/SameSite cookie issuance and authenticated WebUI delivery; session expiry/invalidation is also covered | `ClientWebServer`, `WebRpcClient`, `WebRpcService`, `WebApiRouter`, `WebSessionStore` | Full browser and multiplayer RPC verification remains | Run authenticated identity, mutation and disconnect acceptance tests |
