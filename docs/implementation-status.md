...
| One-time audio delivery | Implemented, partially tested | Pure tests verify token lifecycle plus playback metadata and maximum-sized transfer-chunk packet round trips; oversized chunks are rejected | `TokenStore`, `PlayAudioPacket`, `AudioTransferChunkPacket`, `AudioTransferService` | Live Minecraft packet delivery remains untested; cached clients leave their one-time token unused until expiry | Avoid issuing tokens on confirmed hits later |
