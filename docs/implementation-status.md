...
| One-time audio delivery | Implemented, partially tested | Pure tests verify the 60-second boundary, single consumption under concurrent callers, explicit revocation and null rejection; content hashes are sent with new entries | `TokenStore`, `AudioTransferService`, `SpeakerBlockEntity` | Minecraft packet delivery remains untested; cached clients leave their one-time token unused until expiry | Add packet integration tests and avoid issuing tokens on confirmed hits later |
...
| Playback sequence | Implemented, partially tested | Tests cover metadata CRUD, safe IDs, PULSE/LEVEL transitions, STOP/RESTART retriggers, ordered expansion, due-tick boundaries, LEVEL loop completion and Ogg duration parsing; Controller playback honors post-track delay | `LibrarySequence`, `AudioControllerBlockEntity`, `ControllerSignalTransition`, `ControllerQueuePlan`, `ControllerQueueTiming` | Playback side effects remain uncovered | Add headless integration coverage with controlled audio fixtures |
| Subtitle HUD | Partial | Pure tests cover text normalization, minimum duration, final fade, source-specific clearing and replacement of the single title/subtitle overlay | `SubtitleOverlayManager`, `SubtitleHudRenderer` | Not a multi-cue timed `SubtitleTrack`; packet duration is fixed at 100 ticks; rendering remains untested | Define a compatible timed-track extension only if needed |
...
| Automated tests | Partial | 93 pure-Java tests pass; three Forge GameTests boot a headless 1.20.1 server and verify Speaker BlockEntity creation plus Speaker/Controller NBT round trips | Test sources under `common`, `tts-addon` and `forge/src/gametest` | Actual playback, browser RPC and multi-client paths remain uncovered | Add controller playback and packet integration tests |
