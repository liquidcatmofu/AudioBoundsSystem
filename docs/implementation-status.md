...
| Server lifecycle | Implemented but untested | Minecraft audio/RPC, ffmpeg-probe and cache-maintenance executors are owned and stopped on server stopping; Web RPC scheduling is isolated from blocking API workers | `ABSServerLifecycle`, `AudioTransferService`, `WebRpcService`, `FfmpegSupport`, `LibraryCacheMaintenance` | Requires runtime stop/restart verification; provider bridge has no shutdown hook | Add lifecycle integration tests |
