...
| Server lifecycle | Implemented, partially tested | Audio transfer start/stop is tested for idempotence, restart and token revocation; FFmpeg conversion and startup probes share tested timeout/termination handling; owned executors stop with the server | `ABSServerLifecycle`, `AudioTransferService`, `WebRpcService`, `FfmpegSupport`, `LibraryCacheMaintenance` | Full server runtime stop/restart remains unverified; provider bridge has no shutdown hook | Extend lifecycle tests to the remaining services |
