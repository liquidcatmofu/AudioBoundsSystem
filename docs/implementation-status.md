...
| VOICEVOX-compatible synthesis | Verified working | Handoff runtime report; bounded-reader and loopback HTTP fixture tests cover response sizes, HTTP errors and malformed JSON | `VoiceVoxCompatibleProvider`, `TTSSynthesisException` | Blocking calls occupy a Web RPC worker; live timeout and connection-refused cases remain manual | Verify timeout and unavailable-engine messages at runtime |
