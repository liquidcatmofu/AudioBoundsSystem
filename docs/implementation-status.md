...
| Playback sequence | Implemented, partially tested | Pure tests cover PULSE/LEVEL transitions, STOP/RESTART retriggers and ordered sequence expansion; Controller playback honors each sequence step's post-track delay | `LibrarySequence`, `AudioControllerBlockEntity`, `ControllerSignalTransition`, `ControllerQueuePlan` | Playback side effects and tick-driven advancement remain uncovered | Add headless integration coverage with controlled audio fixtures |
